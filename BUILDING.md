...
git clone --recurse-submodules https://github.com/qbism/4TAK.git


Prerequisities
--------------

4TAK can be built on Linux, BSD and similar platfroms using a recent version
of GCC or Clang.

4TAK client requires either SDL2 or OpenAL for sound output. For video output,
...
To install the *full* set of dependencies for building 4TAK on Debian or
...
4TAK uses Meson build system for its build process.
...
4TAK specific options are listed in `Project options` section. They are
...
Run `sudo ninja -C builddir install` to install 4TAK system-wide into
configured prefix (`/usr/local` by default).

Copy `baseq2/pak*.pak` files and `baseq2/players` directory from unpacked
Quake 2 data into `/usr/local/share/4TAK/baseq2` to complete the
installation.
...
On Windows, 4TAK automatically sets current directory to the directory 4TAK
executable is in. On other platforms current directory must be set before
launching 4TAK executable if portable version is built.


Music support
-------------

4TAK supports playback of background music ripped off original CD in Ogg
...
Library dependencies that 4TAK uses have been prepared as Meson subprojects
...
It is possible to build 4TAK on Windows using Visual Studio 2022 and Meson.
...
Change to 4TAK source directory, then setup build directory:
